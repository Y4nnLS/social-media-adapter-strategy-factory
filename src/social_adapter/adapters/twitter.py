...
class TwitterAPI:
    ...
    def _check(self):
        if not self.enabled:
            raise SimulatedAPIError("Twitter desabilitado via configuração.")
        if not self.api_key or not self.api_secret:
            raise SimulatedAPIError("Credenciais inválidas/ausentes para Twitter.")

    def post(self, text: str, media: Optional[list] = None) -> Dict[str, str]:
        self._check()
        # Simula criação de um ID e link público
        exid = "twitter_" + uuid.uuid4().hex[:8]
        return {"id": exid, "link": f"https://twitter.example.com/p/" + exid}

    def schedule(self, text: str, when: datetime, media: Optional[list] = None) -> Dict[str, str]:
        self._check()
        exid = "sched_twitter_" + uuid.uuid4().hex[:8]
        return {"id": exid, "link": f"https://twitter.example.com/s/" + exid, "when": when.isoformat()}
