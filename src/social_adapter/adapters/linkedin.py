...
class LinkedInAPI:
    ...
    def _check(self):
        if not self.enabled:
            raise SimulatedAPIError("LinkedIn desabilitado via configuração.")
        if not self.api_key or not self.api_secret:
            raise SimulatedAPIError("Credenciais inválidas/ausentes para LinkedIn.")

    def post(self, text: str, media: Optional[list] = None) -> Dict[str, str]:
        self._check()
        # Simula criação de um ID e link público
        exid = "linkedin_" + uuid.uuid4().hex[:8]
        return {"id": exid, "link": f"https://linkedin.example.com/p/" + exid}

    def schedule(self, text: str, when: datetime, media: Optional[list] = None) -> Dict[str, str]:
        self._check()
        exid = "sched_linkedin_" + uuid.uuid4().hex[:8]
        return {"id": exid, "link": f"https://linkedin.example.com/s/" + exid, "when": when.isoformat()}
...
class LinkedInAdapter(SocialMediaAdapter):
    ...
    def __init__(self, api: LinkedInAPI):
        self.api = api
